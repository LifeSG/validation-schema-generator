...
export const otpVerificationField: IFieldGenerator<IOtpVerificationFieldSchema> = (id, { type, validation }) => {
	const otpTypeRule = validation?.find((rule) => "otp-type" in rule) ?? { "otp-type": type };
	...
	return {
		[id]: {
			yupSchema: Yup.object({ contact: contactSchema }).test(
				"is-otp-verified",
				requiredRule?.errorMessage || defaultVerificationError,
				(val) => {
